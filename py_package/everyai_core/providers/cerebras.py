...
Queries Cerebras's API directly using httpx REST endpoints.
"""
...
class CerebrasProvider(BaseProvider):
    ...
    def chat(
        ...
        **kwargs
        ...
        base = (self.base_url or "https://api.cerebras.ai/v1").rstrip("/")
        ...
        if not stream:
            ...
            if response.status_code != 200:
                raise_for_status("cerebras", response.status_code, response.text)
                ...
                try:
                    with client.stream("POST", endpoint, headers=headers, json=payload, timeout=timeout) as r:
                        if r.status_code != 200:
                            r.read()
                            raise_for_status("cerebras", r.status_code, r.text)
