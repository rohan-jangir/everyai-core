...
Queries Mistral's API directly using httpx REST endpoints.
"""
...
class MistralProvider(BaseProvider):
    """Provider class for Mistral LLM API."""
    ...
    def chat(
        ...
        **kwargs
        ...
        """Send a chat completion request to Mistral API using direct HTTP REST."""
        base = (self.base_url or "https://api.mistral.ai/v1").rstrip("/")
        ...
        # Normalize message structures if Mistral requires role keys strictly
        payload = {
            "model": model,
            "messages": messages,
        ...
        if not stream:
            ...
            if response.status_code != 200:
                raise_for_status("mistral", response.status_code, response.text)
                ...
                try:
                    with client.stream("POST", endpoint, headers=headers, json=payload, timeout=timeout) as r:
                        if r.status_code != 200:
                            r.read()
                            raise_for_status("mistral", r.status_code, r.text)
    ...
    def list_models(self) -> list[ModelInfo]:
        """List models available for Mistral."""
        base = (self.base_url or "https://api.mistral.ai/v1").rstrip("/")
