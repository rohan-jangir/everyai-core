...
Queries Groq's high-speed API directly using httpx REST endpoints.
"""
...
class GroqProvider(BaseProvider):
    """Provider class for Groq LLM API."""
    ...
    def chat(
        ...
        **kwargs
        ...
        """Send a chat completion request to Groq API using direct HTTP REST."""
        ...
        # Resolve timeout dynamic parameter
        ...
        if not stream:
            ...
            if response.status_code != 200:
                raise_for_status("groq", response.status_code, response.text)
                ...
                try:
                    with client.stream("POST", endpoint, headers=headers, json=payload, timeout=timeout) as r:
                        if r.status_code != 200:
                            ...
                            r.read()
                            raise_for_status("groq", r.status_code, r.text)
    ...
    def list_models(self) -> list[ModelInfo]:
        """List models available for Groq."""
        base = (self.base_url or "https://api.groq.com/openai/v1").rstrip("/")
