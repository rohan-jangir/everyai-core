"""Dashboard server and UI interface for everyai-core telemetry.

Hosts a lightweight HTTP server serving a premium, modern, slate-dark 
telemetry dashboard with brand-tailored colors, smooth animations, and glassmorphic widgets.
"""
        ...
        function createTableRow(row, showMetaColumn) {
            ...
            if (showMetaColumn) {
                ...
                if (row.error_message) {
                    ...
                    actionBtn = `
                        <button onclick="showErrorModalById(${row.id}, '${row.status.toUpperCase()}')" class="px-2 py-1 bg-slate-800 hover:bg-slate-700 text-teal-400 text-[10px] font-semibold rounded border border-slate-700 transition active:scale-95">
                            Show Error
                        </button>
                    `;
                }
            ...
            errors.forEach(err => {
                ...
                card.innerHTML = `
                    <div class="incident-details">
                        <h4>${errorTitle} <span class="font-bold text-slate-200 ml-2">${err.provider.toUpperCase()}</span></h4>
                        <p class="font-mono text-[11px] text-slate-400">Model: <code class="text-teal-400 font-medium">${err.model}</code> • ${err.timestamp.replace("T", " ")}</p>
                        <div class="incident-msg">${escapeHtml(err.error_message)}</div>
                    </div>
                    <div>
                        <button onclick="showErrorModalById(${err.id}, '${err.status.toUpperCase()}')" class="px-3 py-1.5 bg-slate-850 hover:bg-slate-800 border border-slate-800 rounded-lg text-[10px] font-semibold text-slate-300 transition">
                            Log Trace
                        </button>
                    </div>
                `;
...
class DashboardHandler(http.server.BaseHTTPRequestHandler):
    """Local HTTP Request Handler serving static index.html page and telemetry JSON APIs."""

    tracker: ClassVar[UsageTracker | None] = None

    def log_message(self, format: str, *args: Any) -> None:
        """Silence HTTP network logs to avoid terminal clutter."""
        pass
    ...
    def do_GET(self) -> None:
        ...
        if self.path == "/":
            ...
            if not self.tracker:
                self.send_error(500, "Tracker database is not configured.")
                return
            self.send_response(200)
            ...
            self.send_header("Content-type", "application/json")
            self.end_headers()
            logs = self.tracker.get_logs(limit=200)
...
def start_dashboard(port: int = 8080, db_path: str | None = None) -> None:
    """Start the EveryAI Telemetry dashboard local web server and open the UI in a browser.

    Args:
        port: Local port to host the server.
        db_path: Path to the telemetry tracker SQLite database.
    """
    # Assign the tracker class variable for request handler mapping
    DashboardHandler.tracker = UsageTracker(db_path=db_path)

    # Timer helper to launch default browser shortly after server startup
    def launch_browser() -> None:
        webbrowser.open(f"http://localhost:{port}")

    Timer(0.8, launch_browser).start()

    from http.server import ThreadingHTTPServer
    server_address = ("", port)

    print(f"\n=======================================================")
    print(f"Starting EveryAI Telemetry Dashboard")
    print(f"URL: http://localhost:{port}")
    print(f"Telemetry DB: {DashboardHandler.tracker.db_path}")
    print(f"=======================================================")
    print("Press Ctrl+C to stop the dashboard server.")

    try:
        with ThreadingHTTPServer(server_address, DashboardHandler) as server:
